...
export default function AddResult({ entryToEdit, onSaveSuccess }: AddResultProps) {
    ...
    const validateField = useCallback((key: string, value: string): string | undefined => {
        if (key === 'name') {
            if (!value.trim()) {
                return t('add.validation.nameRequired');
            }
            if (value.length > 25) {
                return t('add.validation.nameTooLong', { maxLength: 25 });
            }
        } else {
            const v = value.trim();
            if (v !== "" && (!/^\d+$/.test(v) || Number(v) < 0 || Number(v) > 100)) {
                return t('add.validation.valueBetween');
            }
        }
        return undefined;
    }, [t]);
    ...
    const handleSubmit = async (e?: React.FormEvent) => {
        ...
        const url = isEditMode ? `http://127.0.0.1:8000/exam/${entryToEdit.id}` : "http://127.0.0.1:8000/exam/save";
